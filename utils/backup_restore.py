...
class BackupRestoreManager:
    ...
    def create_backup(self, backup_type: str = 'FULL', 
        ...
        try:
            ...
            # Partial table backups are intentionally supported. A child-only
            # snapshot can be restored into an existing database without
            # replacing its parent rows; the selective restore path below
            # preserves that workflow while refusing destructive parent swaps.
    ...
    def restore_backup(self, backup_path: str, merge: bool = False) -> Tuple[bool, str]:
        ...
        try:
            ...
            try:
                # Validate the actual SQLite payload before any destructive
                # replacement or merge. The dialog validates imported files,
                # but this manager API is also called directly by the backup
                # list and must never copy a corrupt database into place.
                is_valid, validation_error, _preview = self.validate_backup_file(
                    str(backup_path)
                )
                if not is_valid:
    ...
    def validate_backup_file(self, backup_path: str) -> Tuple[bool, str, Optional[Dict]]:
        ...
        try:
            ...
            # Check the SQLite page structure before inspecting rows.  A file
            # can have the expected header and tables while still containing a
            # damaged page; integrity_check catches that case before restore.
            cursor.execute("PRAGMA integrity_check")
            integrity_result = cursor.fetchone()
            if not integrity_result or integrity_result[0] != 'ok':
                conn.close()
                detail = integrity_result[0] if integrity_result else 'unknown integrity error'
                return False, f"Invalid or corrupted database file: {detail}", None
    ...
    def _restore_sqlite_backup(self, backup_path: Path, backup_info: Optional[Dict] = None) -> bool:
        ...
        current_db_path = Path(DatabaseConfig.get_db_path())
        backup_current = current_db_path.with_suffix('.sqlite.bak')
        temporary_path = current_db_path.with_name(
            f".{current_db_path.name}.restore_tmp"
        )
        replaced = False
        try:
            # Close the active connection before replacing the file. Copy to a
            # sibling temporary file first so a failed copy never truncates the
            # live database.
            DatabaseConfig.close_connection()
            ...
            os.replace(temporary_path, current_db_path)
            replaced = True

            # Reopen the restored database as an operational smoke check.
            conn = DatabaseConfig.get_connection()
            conn.execute("SELECT 1").fetchone()
            return True
        except Exception as e:
            logger.error(f"Error restoring SQLite backup: {e}")
            # Restore the pre-restore file if replacement or reconnect failed.
            try:
                DatabaseConfig.close_connection()
                if replaced and backup_current.exists():
                    os.replace(backup_current, current_db_path)
                DatabaseConfig.get_connection()
            except Exception as rollback_error:
                logger.error(f"Could not roll back failed restore: {rollback_error}")
            return False
        finally:
            try:
                ...
                logger.warning(f"Could not remove temporary restore file: {temporary_path}")
