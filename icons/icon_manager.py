"""
Modern Icon Manager
Uses real SVG/PNG assets for every application icon. Asset loading is centralized
so theme and packaging behavior stays consistent.
"""
...
# Icon name to graphical asset mapping
# Maps button names to asset file names (without extension)
# Application icons are resolved to graphical assets in icons/images/. Aliases
# below keep existing button names stable while avoiding duplicate resources.
ICON_NAME_MAP = {
    # CRUD buttons
    'btn_add': 'add',
    ...
    'btn_execute': 'play',
    'btn_filter': 'filter',
    'btn_expand': 'chevron_down',
    'btn_collapse': 'chevron_up',
...
def get_file_type_icon(extension: str, size: int = 32) -> QIcon:
    ...
    # Use the same theme-aware loader as action icons so attachment icons
    # remain visible in dark mode as well.
...
def get_icon(button_name: str, size: int = 24, use_white: bool = False) -> QIcon:
    """
    Get an icon for a button name from the centralized graphical asset store.
    ...
    # Keep vector icons legible when the application switches to its dark
    # theme. The import is lazy to avoid a styles/icons import cycle.
    dark_theme = False
    try:
        from styles.styles import AppStyles
        dark_theme = AppStyles.is_dark_theme()
    except Exception:
        pass
    effective_white = use_white or dark_theme
    icon_color = '#F5F7FA' if effective_white else '#2c3e50'

    # All production icons must come from a graphical asset. The generic file
    # icon is an intentional final fallback for a missing alias or a packaged
    # build that contains a newly added control before its specialized asset is
    # available; it is never replaced by a text/font glyph or a QPainter shape.
    file_icon = _file_manager.load_icon_from_file(icon_name, size, icon_color)
    if not file_icon.isNull():
        return file_icon

    generic_icon = _file_manager.load_icon_from_file('file_generic', size, icon_color)
    if not generic_icon.isNull():
        return generic_icon

    # A null icon is preferable to silently rendering a character or a
    # programmatically drawn substitute. Build verification should catch this
    # missing-resource condition before release.
    return QIcon()


def setup_icon_label(label, icon_name: str, size: int = 18, tooltip_text: str = ""):
    """Apply a graphical icon to a QLabel used as an icon-only/status marker.

    Labels that carry explanatory text should use a separate text label next to
    this marker. This helper keeps icon loading centralized and provides a
    consistent accessible tooltip for status icons.
    """
    ...
    label.setAccessibleName(tooltip_text or icon_name.replace('_', ' ').title())
    return label
...
def setup_icon_button(button, button_name: str, tooltip_text: str, size: int = 26, use_white_icon: bool = False):
    ...
    if not icon.isNull():
        ...
        # A missing asset must never be replaced by a text glyph or a
        # Unicode/ASCII pseudo-icon. Keep any descriptive label supplied by
        # the caller, but do not synthesize one from the tooltip.
