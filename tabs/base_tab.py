...
class SimpleTableWidget(QTableWidget):
    ...
    def setup_table(self):
        ...
        # Extended selection supports review workflows and bulk actions while
        # preserving current-row CRUD behavior for Edit/Delete.
        self.setSelectionMode(QAbstractItemView.ExtendedSelection)
        self.setEditTriggers(QTableWidget.NoEditTriggers)
        self.setFocusPolicy(Qt.StrongFocus)
        self.setTextElideMode(Qt.ElideRight)
        self.setContextMenuPolicy(Qt.CustomContextMenu)
        # Sorting is handled by the owning tab so it covers the complete
        # filtered result, not only the currently visible pagination page.
        self.setSortingEnabled(False)
        self.verticalHeader().setVisible(False)

        # Keep rows compact; the full cell value remains available through the
        # tooltip and the preview panel.
        self.setWordWrap(False)
        self.setRowHeight(0, 38) if self.rowCount() else None
    ...
    def refresh_display(self):
        ...
        for row_idx, row_data in enumerate(self.display_data):
            ...
            # Keep the complete row on the item so preview and actions remain
            # correct after the user sorts the table. The record id is still
            # available through get_selected_id().
            ...
            for col_idx, (col_key, _, _) in enumerate(self.original_columns):
                ...
                # Keep the grid dense while making truncation recoverable.
                item.setToolTip(display_value)
                self.setItem(row_idx, col_idx + 1, item)

            self.setRowHeight(row_idx, 38)

        # Keep native row sorting disabled; the tab owns sorting so that a
        # click applies to the complete filtered dataset before pagination.
        self.setSortingEnabled(False)

        # Select an initial/reloaded result for the preview, but do not replace
        # a user's selection simply because they changed page or page size.
        if len(self.display_data) > 0 and self._select_first_after_render:
    ...
    def _select_first_row(self):
        ...
        if self.rowCount() > 0 and not self.selectionModel().selectedRows():
            self.selectRow(0)
    ...
    def get_selected_id(self) -> Optional[int]:
        ...
        if current_row >= 0:
            ...
            if item:
                ...
                # Preserve compatibility with tables populated by older
                # callers that stored the id directly.
                return row_data
        return None
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as f:
            if self.filtered_data:
                if selected_columns:
                    ...
                    writer.writerow(sanitize_row({col[0]: col[1] for col in selected_columns}))
...
class BaseTableTab(QWidget):
    ...
    def get_toolbar_config(self) -> ToolbarConfig:
        ...
        # Column visibility is a view preference rather than a primary record
        # action, so it intentionally lives in the overflow menu.
        buttons.append(ButtonConfig(
    ...
    def get_callbacks(self) -> Dict[str, callable]:
        ...
        callbacks = {
            ...
            'column_visibility': self.toggle_column_visibility,
    ...
    def setup_ui(self):
        ...
        self.selection_clear_button = QPushButton()
        ...
        # Handle sorting at the tab level so the complete filtered result is
        # ordered before pagination rather than sorting only the visible page.
    ...
    def on_selection_changed(self):
        ...
        if current_row >= 0:
            ...
            if not isinstance(row_data, dict) and current_row < len(display_data):
            ...
            self.preview_panel.clear_preview()
        # Selection-sensitive actions and the selected-count context must be
        # refreshed even when the current row has valid preview data.
        self._update_action_state()
        self._update_result_summary()

    def _update_action_state(self):
        """Keep selection-sensitive actions honest without hiding capabilities."""
        has_selection = bool(self.data_table.selectedItems()) if hasattr(self, 'data_table') else False
        for key in ('btn_edit', 'btn_delete'):
            button = self.toolbar_factory.get_button(key) if hasattr(self, 'toolbar_factory') else None
            if button:
                button.setEnabled(has_selection)
        # Bulk operations and advanced search remain usable with no selection;
        # their menu entries are intentionally not disabled.

    def _update_result_summary(self):
        """Update the compact context line for filtered and selected results."""
        if not hasattr(self, 'result_summary_label'):
            return
        filtered = len(getattr(self.data_table, 'filtered_data', []) or [])
        total = len(getattr(self.data_table, 'data', []) or [])
        selected_rows = len(self.data_table.selectionModel().selectedRows()) if hasattr(self, 'data_table') else 0
        records = self.translator.tr('lbl_records', default='records')
        selected = self.translator.tr('table_selected', default='selected')
        if filtered != total:
            self.result_summary_label.setText(
                f"{filtered} {records} · {total} {self.translator.tr('table_total', default='total')} · "
                f"{selected_rows} {selected}"
            )
        else:
            self.result_summary_label.setText(f"{total} {records} · {selected_rows} {selected}")
        if hasattr(self, 'selection_action_bar'):
            self.selection_action_bar.setVisible(selected_rows > 0)
            self.selection_count_label.setText(
                self.translator.tr('bulk_selected_count', count=selected_rows)
            )

    def _set_table_state(self, state: str, message: str = '', recoverable: bool = False):
        """Show a reusable empty/loading/error state next to the table."""
        ...
        text = message or default_messages.get(state, default_messages['error'])
        if recoverable:
            text += f"  {self.translator.tr('table_recover', default='Use Refresh to try again.')}"
        self.table_state_label.setText(text)
        self.table_state_label.setProperty('state', state)
        self.table_state_label.setVisible(True)
        self.table_state_label.style().unpolish(self.table_state_label)
        self.table_state_label.style().polish(self.table_state_label)

    def _show_table_context_menu(self, position):
        """Provide keyboard/mouse access to common row and review actions."""
        ...
        menu.addSeparator()
        preview_action = QAction(get_icon('btn_preview', 18), self.translator.tr('btn_preview', default='Preview'), menu)
        preview_action.setEnabled(bool(self.data_table.currentItem()))
        preview_action.triggered.connect(self.on_selection_changed)
        menu.addAction(preview_action)
        edit_action = QAction(get_icon('btn_edit', 18), self.translator.tr('btn_edit'), menu)
        edit_action.setEnabled(bool(self.data_table.currentItem()))
    ...
    def _sort_value(value):
        """Return a stable key for mixed database values."""
        if value is None:
            return (2, '')
    ...
    def _sort_filtered_rows(self, rows):
        """Sort rows using the current header sort state."""
        if self._sort_column is None:
            return list(rows)

        if self._sort_column == 0:
            # The row-number column represents the current result order.
    ...
    def on_table_sort_requested(self, column: int):
        """Sort the full filtered result when a table header is clicked."""
        ...
        self._full_filtered_data = self._sort_filtered_rows(
            getattr(self.data_table, 'filtered_data', [])
        )
        self.data_table.filtered_data = list(self._full_filtered_data)
    ...
    def apply_filters(self):
        ...
        # matching row. Reapply the user's sort after every filter change.
        self.data_table._select_first_after_render = True
        self.data_table.filtered_data = self._sort_filtered_rows(
    ...
    def _deferred_load_data(self):
        ...
        if not self._data_loaded and self.isVisible():
            try:
                ...
                if not getattr(self.data_table, 'filtered_data', []):
    ...
    def set_header(self):
        """Open the shared print-header settings dialog.

        All table tabs expose this toolbar action, so the base implementation
        keeps the control functional even when a tab does not need a custom
        header workflow.
        """
        try:
            ...
            if dialog.exec_() == QDialog.Accepted:
                self.status_label.setText(self.translator.tr('msg_header_saved'))
        except Exception as exc:
            logger.error(f"Error opening header settings: {exc}")
            QMessageBox.critical(
                self,
                self.translator.tr('msg_error'),
                self.translator.tr('msg_header_save_failed')
            )
    ...
    def _export_csv_data(self, filepath: str, data: List[Dict], columns: List[tuple]):
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as f:
            if data:
                ...
                writer = csv.DictWriter(f, fieldnames=fieldnames)
                # Write header with display names and protect user-controlled
                # values from spreadsheet formula evaluation.
    ...
    def toggle_column_visibility(self):
        """Show a reversible column-visibility control for the active table."""
        ...
        checks = []
        for index, (_, label, _) in enumerate(self.data_table.columns):
            if index == 0:
                continue  # Row numbers are always available for orientation.
    ...
    def show_advanced_search(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            results = list(dialog.get_results() or [])
            # An accepted search with zero matches is still a real result and
            # must clear the table instead of leaving stale rows visible.
            self.data_table.data = list(results)
            self.data_table.filtered_data = list(results)
            self.data_table.display_data = list(results)
            self._full_filtered_data = list(results)
            self.pagination.current_page = 1
    ...
    def refresh_translations(self):
        ...
        # Reapply theme-aware styles that are intentionally scoped to the
        # reusable table workspace. This keeps light/dark changes consistent
        # even though the controls were created before the theme was switched.
        if hasattr(self, 'unified_scroll'):
            self.unified_scroll.setStyleSheet(AppStyles.get_unified_scroll_area_style())
        ...
        if hasattr(self, 'toolbar_factory'):
            ...
            date_group = self.toolbar_factory.buttons.get('date_filter_group')
            if date_group:
                date_group.setStyleSheet(AppStyles.get_component_style('date_filter_group'))
            for label_key in ('search_label', 'date_from_label', 'date_to_label'):
                label = self.toolbar_factory.buttons.get(label_key)
                if label:
        ...
        if hasattr(self, 'status_label'):
            ...
            if hasattr(self, 'table_state_label'):
                ...
                if self.table_state_label.isVisible():
                    current_state = self.table_state_label.property('state') or 'empty'
                    self.table_state_label.setText(self.translator.tr(
                        f'table_{current_state}', default=self.table_state_label.text()
                    ))
            if hasattr(self, 'pagination'):
                self.pagination.update_display()

        if hasattr(self, 'selection_bulk_button'):
            self.selection_bulk_button.setText(self.translator.tr('bulk_operations', default='Bulk operations'))
            self.selection_bulk_button.setToolTip(self.selection_bulk_button.text())
            self.selection_bulk_button.setAccessibleName(self.selection_bulk_button.text())
        if hasattr(self, 'selection_clear_button'):
            self.selection_clear_button.setText(self.translator.tr('btn_clear', default='Clear'))
            self.selection_clear_button.setToolTip(self.selection_clear_button.text())
            self.selection_clear_button.setAccessibleName(self.selection_clear_button.text())
        ...
        if hasattr(self, 'pagination'):
            self.pagination.setStyleSheet(AppStyles.get_premium_pagination_style())
            if hasattr(self.pagination, 'refresh_translations'):
