    ...
    def _show_table_view(self):
        ...
        if self.chart_data:
            ...
            for row in self.chart_data[:15]:  # Limit to 15 rows
                row_values = [str(row.get(col, ''))[:20] for col in columns]  # Truncate values
...
class ReportsTab(QWidget):
    ...
    def create_toolbar(self, parent_layout):
        ...
        self.btn_new = QPushButton(self.translator.tr('report_new'))
        self.btn_new.setSizePolicy(QSizePolicy.Minimum, QSizePolicy.Preferred)
        self.btn_new.clicked.connect(self.new_report)
        
        self.btn_save = QPushButton(self.translator.tr('report_save'))
        self.btn_save.setSizePolicy(QSizePolicy.Minimum, QSizePolicy.Preferred)
        self.btn_save.clicked.connect(self.save_report)
        
        self.btn_load = QPushButton(self.translator.tr('report_load'))
        self.btn_load.setSizePolicy(QSizePolicy.Minimum, QSizePolicy.Preferred)
        ...
        setup_icon_button(self.btn_print, 'btn_print', self.translator.tr('btn_print'))
        self.btn_print.clicked.connect(self.print_report)
        
        self.btn_print_preview = QPushButton()
        setup_icon_button(self.btn_print_preview, 'btn_print_preview', self.translator.tr('btn_print_preview'))
        self.btn_print_preview.clicked.connect(self.print_preview)
        
        self.btn_export_pdf = QPushButton()
        setup_icon_button(self.btn_export_pdf, 'btn_export_pdf', self.translator.tr('btn_export_pdf'))
        self.btn_export_pdf.clicked.connect(self.export_pdf)
        
        self.btn_export_excel = QPushButton()
        setup_icon_button(self.btn_export_excel, 'btn_export_excel', self.translator.tr('btn_export_excel'))
        self.btn_export_excel.clicked.connect(self.export_excel)
        
        self.btn_export_csv = QPushButton(self.translator.tr('btn_export_csv'))
        self.btn_export_csv.setSizePolicy(QSizePolicy.Minimum, QSizePolicy.Preferred)
        self.btn_export_csv.clicked.connect(self.export_csv)
        
        toolbar.addWidget(self.btn_print)
        toolbar.addWidget(self.btn_print_preview)
        ...
        toolbar.addWidget(QLabel(self.translator.tr('report_orientation') + ":"))
        toolbar.addWidget(self.orientation_combo)
        toolbar.addWidget(QLabel(self.translator.tr('report_page_size') + ":"))
    ...
    def create_results_tab(self) -> QWidget:
        ...
        layout.addWidget(self.results_table, 1)  # stretch factor 1 to take available space
        ...
        self.btn_chart_from_data = QPushButton(self.translator.tr('report_create_chart'))
    ...
    def on_query_executed(self, data: List[Dict], columns: List[str]):
        ...
        # Update results table with row numbering
        ...
        for row_idx, row_data in enumerate(data):
            ...
            self.results_table.setItem(row_idx, 0, row_num_item)
            
            # Add data columns (starting from column 1)
            for col_idx, col_name in enumerate(columns):
                raw_value = row_data.get(col_name)
                # Handle None/NULL values - display as empty or dash
                if raw_value is None:
                    value = '-'
                elif isinstance(raw_value, float):
                    # Format floats nicely (e.g., importance as percentage)
        ...
        # Set fixed width for row number column
        self.results_table.setColumnWidth(0, 50)
        
        # Update stats
        self.results_info.setText(f"{self.translator.tr('report_results')}: {len(data)} {self.translator.tr('report_rows')}")
        ...
        try:
            if export_format == ExportPreviewDialog.FORMAT_EXCEL:
                ...
                with open(filepath, 'w', newline='', encoding='utf-8-sig') as f:
                    ...
                    if include_header and print_settings:
                        if print_settings.header_left_line1:
                            writer.writerow([print_settings.header_left_line1])
                        if print_settings.header_left_line2:
                            writer.writerow([print_settings.header_left_line2])
